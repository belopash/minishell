...
/*   Updated: 2021/01/22 12:31:48 by ashea            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int ft_if_double_quotes_dop(int *j, char *content, char *line, t_shell *shell)
{
    int i;
    int delta;
    ...
    while (line[i] != '"' && line[i])
    {
        if ((delta = ft_if_env(j, content, line + i, shell)) > 0)
        {
            i += delta;
            continue;
        }
...
static int ft_if_empty_double_quotes(int *i, int *j, char *content, char *line)
{
...
int ft_if_double_quotes(int *j, char *content, char *line, t_shell *shell)
{
    ...
    if (line[i] == '"')
    {
        ...
        if (ft_if_empty_double_quotes(&i, j, content, line))
            return (i);
        i += ft_if_double_quotes_dop(j, content, line + i, shell);
        if (line[i] == '\0')
