...
static int	ft_redirect_l(int *i, char *line,
...
static int	ft_redirect_r(int *i, char *line,
		t_command *command, t_shell *shell)
{
	if (line[*i] == '>')
	{
...
int			ft_if_redirect(int *i, char *line,
		t_command *command, t_shell *shell)
{
	if (ft_redirect_r(i, line, command, shell) ||
			ft_redirect_l(i, line, command, shell))
		return (1);
	return (0);
}
