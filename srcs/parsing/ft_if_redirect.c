...
/*   Updated: 2021/01/22 12:27:05 by ashea            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/parsing.h"

static int ft_redirect_l(int *i, char *line, t_command *command, t_shell *shell)
{
    if (line[*i] == '<')
       {
           (*i)++;
...
static int ft_redirect_r(int *i, char *line, t_command *command, t_shell *shell)
{
    if (line[*i] == '>')
       {
           (*i)++;
...
{
    if (ft_redirect_r(i, line, command, shell) || ft_redirect_l(i, line, command, shell))
        return(1);
    return (0);
}
